    ...
    <div class="sm:w-1/3 lg:w-1/2 flex flex-col text-center items-center">
        ...
        <%= if @letter.status != :zarezerwowany do %>
          <div class="mt-6" x-data="{ open: false }">
            <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold px-8 border border-gray-400 rounded shadow align-middle" @click="open = true">Zarezerwuj</button>
              ...
              <div class="relative z-2 w-7/12 max-w-3xl bg-white p-8 mx-auto rounded-xl flex flex-col items-center" @click.away="open = false">
                <.form let={f} for={@reservation} url="#" phx_submit={:save} class="w-full max-w-lg" >
                  ...
                  <div class="flex flex-wrap -mx-3 mb-6 justify-evenly">
                    ...
                    <div class="w-full md:w-1/3 bg-white hover:bg-red-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow cursor-pointer text-lg pt-4" @click="open = false">ZAMKNIJ</div>
                  </div>
                </.form>
              </div>
            </div>
